...
static inline void oled_map_xy(uint8_t x, uint8_t y, uint8_t *xo, uint8_t *yo)
{
    *xo = (uint8_t)(OLED_PHY_WIDTH - 1 - y);
    *yo = x;
}
...
void oled_DrawPixel(uint8_t x, uint8_t y, OLED_COLOR color)
{
    ...
    uint8_t xp, yp;
    oled_map_xy(x, y, &xp, &yp);

    uint32_t index = xp + (uint32_t)(yp / 8u) * OLED_PHY_WIDTH;
    uint8_t  mask  = (uint8_t)(1u << (yp % 8u));

    if (color == White) OLED_Buffer[index] |=  mask;
    else               OLED_Buffer[index] &= (uint8_t)~mask;
}
